...
Regras inegociáveis:
1. NUNCA invente informações. Se não souber, peça o WhatsApp do cliente pra Jocimar (o dono) entrar em contato: 5543999919777.
...
- **Não** há informação nutricional impressa na embalagem. Se cliente perguntar sobre alérgenos/restrições, direcione pro WhatsApp 5543999919777 pra equipe confirmar disponibilidade do dia.
- Unidade física: MilkyPot Muffato Quintino — Londrina-PR. Horário 10h–22h. Taxa delivery R$ 5,90. Tempo 25-40 min.
- Owner: Jocimar Rodrigo Magalhães Serra. WhatsApp: 5543999919777 (mesmo do atendimento).
...
## COMO PEDIR
- Pelo cardápio online (cardapio.html) ou direto no WhatsApp 5543999919777.
...
module.exports = async (req, res) => {
    ...
    if (!apiKey) {
        ...
        res.status(503).json({ error: "Chat temporariamente indisponível 😔 Fale comigo no WhatsApp: 5543999919777" });
        return;
    }
    ...
    try {
        ...
        if (!groqResp.ok) {
            ...
            res.status(502).json({ error: "Desculpe, tô com problema de conexão 😔 Fale comigo no WhatsApp: 5543999919777" });
            return;
        }
        ...
        res.status(500).json({ error: "Erro inesperado. Tenta de novo ou fala com a gente no WhatsApp 5543999919777!" });
    }
};
