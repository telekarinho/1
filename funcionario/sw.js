/* Service Worker — Portal do Funcionario MilkyPot
   Cache-first para assets, network-first para HTML, push notifications.
*/
const SW_VERSION = 'mp-func-v1';
const CACHE_NAME = 'milkypot-funcionario-' + SW_VERSION;

const PRECACHE = [
    '/funcionario/',
    '/funcionario/index.html',
    '/funcionario/manifest.json',
...
self.addEventListener('activate', (event) => {
    event.waitUntil(
        caches.keys().then((names) =>
            Promise.all(names.filter((n) => n !== CACHE_NAME && n.startsWith('milkypot-funcionario-')).map((n) => caches.delete(n)))
    ...
    if (request.mode === 'navigate' || request.headers.get('accept')?.includes('text/html')) {
        event.respondWith(
            fetch(request).then((res) => {
                ...
                caches.match(request).then((c) => c || caches.match('/funcionario/index.html'))
            )
        );
        return;
    }
...
self.addEventListener('push', (event) => {
    let data = { title: 'MilkyPot', body: 'Lembrete do seu ponto', url: '/funcionario/' };
    try {
...
self.addEventListener('notificationclick', (event) => {
    ...
    const url = event.notification.data?.url || '/funcionario/';
    event.waitUntil(
        clients.matchAll({ type: 'window', includeUncontrolled: true }).then((winClients) => {
            for (const client of winClients) {
                if (client.url.includes('/funcionario/') && 'focus' in client) return client.focus();
            }
