...
function pareceMensagemBotExterno(text) {
    ...
    // SÓ marca como bot externo se tiver padrão CLARO de cobrança (CPF/boleto/SPC/etc).
    // [mensagem não suportada] NÃO é bot — é cliente real mandando mídia (figurinha,
    // áudio, foto) que Baileys não decodificou. Belinha trata isso como mídia normal
    // e responde de forma acolhedora, NUNCA como bot externo.
    return PATTERNS_BOT_COBRANCA.some(rx => rx.test(text));
}
...
function detectLoop(history, currentText, nextReplyCandidate = null) {
    ...
    // IGNORA quando é media marker — cliente mandando múltiplas figurinhas/
    // fotos/áudios em sequência é NORMAL, não é loop de bot.
    const MEDIA_MARKER_RX = /^\[(sticker|image|imagem|figurinha|audio|áudio|voice|video|vídeo|location|localização|document|documento|contact|contato|gif|reaction|rea[cç][aã]o|mensagem n[aã]o suportada|unsupported message)\]$/i;
    const isCurrentMedia = MEDIA_MARKER_RX.test(String(currentText || "").trim());
    if (userMsgs.length >= 1 && !isCurrentMedia) {
    ...
    // 3. REMOVIDO: detecção de loop por mídia repetida.
    // Cliente real frequentemente manda múltiplas figurinhas/áudios/fotos
    // antes de explicar o pedido. Tratar isso como bot/loop estraga UX.
    // Mídia é tratada no media_marker handler do webhook, que responde
    // de forma acolhedora pedindo texto (sem disparar handoff humano).
